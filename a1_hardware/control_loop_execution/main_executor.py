...
class Executor:
  def __init__(
    ...
    control_freq = 20, frame_interval = 4, Kp = 40, Kd = 0.4,
    use_visual=True
  ):
  ...
  def warmup_observations(self):
    self.last_action = torch.zeros(12, device=self.device, requires_grad=False)
    ...
    for i in range(3):  # sensor history buffer have length 3 at most
      ...
      #print(observation)
      obs = prepare_obs(observation, self.last_action)
  ...
  def main_execution(self):
    ...
    curr_frame, depth_scale = None, None
    while self.continue_thread:
      ...
      # self.robot_controller.set_target_dof_pos(action)
