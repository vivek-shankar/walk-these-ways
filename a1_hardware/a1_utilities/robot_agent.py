...
class Agent():
    def __init__(self, cfg, se, robot_controller, command_profile=None):
    ...
    def set_action(self, action, hard_reset=False):
        ...
        cmd = np.zeros(60, dtype=np.float)
        for i in range(12):
            cmd[i*5] = joint_pos_target[i]
            cmd[i*5+1] = self.p_gains
            cmd[i*5+2] = 0
            cmd[i*5+3] = self.d_gains
            cmd[i*5+4] = 0

        self.torques = (self.joint_pos_target - self.dof_pos) * self.p_gains + (self.joint_vel_target - self.dof_vel) * self.d_gains

        self.robot_controller.set_action(cmd)
    ...
    def step(self, actions, hard_reset=False):
        ...
        self.set_action(self.actions, hard_reset=hard_reset)
