...
class StateEstimator:
    def __init__(self, robot_controller, use_cameras=False):
    ...
    def refresh(self):
        ...
        self.euler[0] = observation.imu.rpy[0]
        self.euler[1] = observation.imu.rpy[1]
        self.euler[2] = observation.imu.rpy[2]
        self.R = get_rotation_matrix_from_rpy(self.euler)
        contact_forces = np.zeros(4, dtype=np.float)
        contact_forces[0] = observation.footForce[0]
        contact_forces[1] = observation.footForce[1]
        contact_forces[2] = observation.footForce[2]
        contact_forces[3] = observation.footForce[3]
        self.contact_state = 1.0 * (contact_forces > 200)
        ...
        self.buf_idx += 1
        self.euler_prev[0] = observation.imu.rpy[0]
        self.euler_prev[1] = observation.imu.rpy[1]
        self.euler_prev[2] = observation.imu.rpy[2]

        for i in range(12):
            self.joint_pos[i] = observation.motorState[i].q
            self.joint_vel[i] = observation.motorState[i].dq
            self.tau_est[i] = observation.motorState[i].tauEst
